...
class DynamicUpdate():
    ...
    def on_launch(self):
        ...
        # self.figure.suptitle('Área fixa')
...
def fitness(aco, rigidez, correct_ans):
    ...
    if(total_weight < 4 or total_weight > 11):
        error = 10**10
...
def generate_new_solution(generation_size):
    _aco = []
    _rigidez = []
    while(len(_rigidez) != generation_size):
        ...
        _rigidez_copy = np.array(_rigidez)
        _aco_copy = np.array(_aco)
        list_rigidez = np.array(list_rigidez)
        _aco_colist_acopy = np.array(list_aco)

        exist = False
        for j in range(len(_rigidez_copy)):
            if(np.array_equal(_rigidez_copy[j], list_rigidez) and np.array_equal(_aco_copy[j], _aco_colist_acopy)):
               exist = True 
...
def crossover_function(bestsolutions):
    ...
    while(len(crossover_rigidez) != 10):
        ...
        iteractions += 1
    # print(f"tamanho cros: {len(crossover_rigidez)}")
    # sys.exit()
...
def mutation_function(elements, generation_size):
    ...
    return new_rigidez_unicos, new_aco_unicos
...
if chart_on:
    plt.savefig('final_sp_2_1_4.pdf', dpi=300)
...
#o que melhorar é verificar se a solução já exite, se ela existe joga fora e tenta outra, se não as melhores vão ser sempre iguais