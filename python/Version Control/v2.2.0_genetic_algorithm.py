#Código semelhante ao 2.1.0, porém para testes de influencia com variação de comprimento e largura, 
#porém no foi feito o crossover para a variação de comprimento e largura (talvez melhore com isso?
#Pois até então a variação de comprimento e largura não se mostrou muito eficaz em termos de melhora 
#de convergencia)
#(16/07/2023)
...
def print_current_best_individual(best_tuple, rankedsolutions, i):
    ...
    for j in range(3):
        ...
        print(f"Accuracy: {round(rankedsolutions[j][0], 4)},   Weight: {round(total_weight, 2)}kg,   error per freq: {[round(er*100, 2) for er in rankedsolutions[j][3]]}")
...
rigidez, aco, comprimento, largura = generate_new_solution(500)
#parameters         [L,C,tmola,Eepdm],  [L,C,taco], [comprimento],  [largura,  [Hz, Hz, Hz, Hz, Hz],   limit_generation,   generation_size
selection_function  (rigidez,           aco,        comprimento,    largura,   frequency_target,       30000,                500)
